...
function update() {
  ...
  if (doodle.vy > 0) doodle.isBoosted = false;
  doodle.y += doodle.vy;
  ...
  while (platforms.length < 12) {
    ...
    // === Smart enemy spawn ===
    const visiblePlats = platforms.filter(p => p.y > 0 && p.y < 600);
    const uniqueZones = visiblePlats.map(p => Math.floor(p.x / 80));
    const zoneCount = uniqueZones.reduce((acc, z) => { acc[z] = (acc[z] || 0) + 1; return acc; }, {});
    const sparse = Object.values(zoneCount).filter(c => c === 1).length >= 3;

    if (score > 5000 && Math.random() < 0.05) {
      const ex = Math.random() * 360;
      const ey = lastY - 100;
      if (canSpawnEnemyHere(ex, ey)) enemies.push({
        x: ex,
        y: ey,
        y: lastY - 100,
        width: 40,
...
function showGameOverMenu() {
  ...
  try {
    fetch("http://173.212.248.252:8000/score", {
      method: "POST",
      headers: {
    ...
    console.error("Failed to post global score", e);
  }
  ...
  document.getElementById("scoreDisplay").innerText = "Score: " + score + "\nBest: " + bestDisplay;
    ...
    function canSpawnEnemyHere(x, y) {
      return platforms.some(p => {
        const dx = Math.abs(p.x - x);
        const dy = Math.abs(p.y - y);
        return dx > 40 && dx < 120 && dy < 60;
      });
    }
